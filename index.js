...
quoted: mek, caption: `Wah ganteng kek gua`
  ...
  break
				...
				case 'tebakgambar':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api-mikubot.herokuapp.com/api/kuis/tebakgambar?apikey=ojanganz`, {method: 'get'})
...
if (isBanned) return reply('```Lu kebanned kontol```')
		...
		case 'quotes': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/quotes?apikey=RamlanID`, {method: 'get'})
		...
		case 'heroml':  
				...
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
					 ige = body.slice(12)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/tiktok?apikey=RamlanID&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group Miku-BOT :\n\n\`\`\``
					no = 0
		...
		case 'bcc': 
				if (!isAdmin) return reply('*Only Admin bot*')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST MIKU-BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
